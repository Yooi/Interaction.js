...
const page = InteractionJS;
//step 1: fill editor .editor-kit-root-container input
await page.inputAfterEnter(".editor-kit-root-container input", '${title}');
...
if(anchortype == '位置' && position){
    await page.click(".semi-select", '输入地理位置');
    await page.inputAfterEnter("div[class*='anchor-item-'] input", position);
    await page.sleep(2000);
    await page.click("div[class*='option-v2-']", position);

}else{
    await page.click(".semi-select", '粘贴抖音小程序链接');
    await page.inputAfterEnter("div[class*='anchor-item-'] input", '${anchorUrl}');
}

//step 3: fill category
if(document.querySelector(".semi-cascader")){
    document.querySelector(".semi-cascader").click();
    await page.click(".semi-cascader-option", '${catelog_name}');
    await page.click(".semi-cascader-option-list:nth-child(2) .semi-cascader-option", '${subcatelog_name}');
    await page.wait('.semi-space').then(element=>{
...
//step 4: fill hotspot
if(hotspot){
    await page.click(".semi-select-selection-text", '点击输入热点词');
    await page.inputAfterEnter(".container--nsGe1 input", '${hotspot_name}');
    await page.sleep(2000);
    await page.click(".semi-select-option", '${hotspot_name}');
}
...
if(album){
    ...
    await page.sleep(2000);
    await page.click(".semi-select-option", '${album_name}'); 
}
...
await page.click("label[class*='radio--']", publish);
if(publish == '定时发布'){
    await page.inputAfterEnter(".semi-datepicker input", '${publish_time}');
}
